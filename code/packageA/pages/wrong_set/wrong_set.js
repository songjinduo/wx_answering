...
//选取收藏的题目
function get_wrong(questionData, list) {
  for (var i = 0; i < list.length; i++) {
    if (list[i] < 10000)
      wrongData.push(questionData.data_all[list[i] - 1])
    else
      wrongData.push(questionData.data_multi[list[i] - 10001])
    //wrongData[i].isStore = true
  }
  return wrongData;
  console.log("以下是错误的题目")
...
Page({
  ...
  tapSelect: function (e) {
    ...
    if (bool) {//答对了
      ...
      /*  pro_suc.push(this.data.answers.allList[this.data.answers.start + this.data.swiper.active].id)*/
      if (result_wro > -1)
        pro_wro.splice(result_wro, 1);
      console.log(this.data.answers.allList[this.data.answers.start + this.data.swiper.active])
      console.log(pro_suc)
    } else {//失败了
...
*/
  ...
  getSubject: function () {
    ...
    if (this.$isLock) {
      this.isLockCall = function () {
        ...
        this.data.answers.allList = wrongData
        this.data.isLoading = false
        this.data.isFirst = true
        this.setData(this.data)
      }
  ...
  onShow: function (params) {
    ...
    wrongData = []
    get_wrong(questionData, pro_wro)
    console.log(data_c)
    for (var i = 0; i < wrongData.length; i++) {
      if (contains(data_c, wrongData[i].id) == -1) {
        wrongData[i].isStore = false
      } else {
        wrongData[i].isStore = true
      }
    }
